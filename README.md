...
| ~~Event household_id is caller-supplied with no membership check~~ | 10 | ✅ Resolved — `/api/events` now looks up `household_id` server-side from `household_members` rather than accepting it from the client. |
...
| Multiple test files import deleted code and fail to load | — | `tests/integration/households.test.ts` imported `GET` from `app/api/households/me/route.ts`, deleted as dead code in June — the whole file failed at import time, so none of its tests (household create/join/invite) had run since then. Fixed as part of the onboarding-resilience PR. At least 8 other test files (`discover`, `feedback`, `recipes-delete`, `recipes`, `share-save`, `formatters`, ...) have the same class of problem — stale imports or type mismatches from since-removed code — and are currently failing `npx vitest run` / `npx tsc --noEmit` on `main`. Needs a dedicated pass to find and fix all of them; a passing test suite should not be assumed without running it. |
...
- Occasion menus — save a named collection of recipes tied to a specific event ("Easter 2025", "Clark's graduation dinner") with a date and optional note; captures what you actually served and how it went, distinct from meal planning which is forward-looking
